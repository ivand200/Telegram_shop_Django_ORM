...
def show_prod(update: Update, context: CallbackContext):
    ...
    keyboard = [[InlineKeyboardButton("Add to Cart", callback_data="add_to_cart")]]
    reply_markup = InlineKeyboardMarkup(keyboard)
    #context.bot.send_photo(update.effective_chat.id, photo=open("1_.png", "rb"),
    #caption=(f"{query}</b>", reply_markup=reply_markup, parse_mode=telegram.ParseMode.HTML)query['data']
    context.bot.send_message(update.effective_chat.id, text=f"<b>{query}, {d}, {title[0]}, {price_}</b>", reply_markup=reply_markup, parse_mode=telegram.ParseMode.HTML)"""
...
def add_product(update: Update, context: CallbackContext):
    ...
    chat_id = query.from_user.id
    username = query.from_user.username
    p = update.callback_query.message.caption
    x = p.split("\n")
    p = Product.objects.filter(name=x[0]).values_list("pk")
    p_ = str(p[0]).replace('(','').replace(')','').replace(',','')
    c, created = Customer.objects.get_or_create(telegram_id=chat_id, defaults={'name': query.from_user.username,})
    user = Customer.objects.filter(name=username).values_list("pk")
    user_ = str(user[0]).replace('(','').replace(')','').replace(',','')
    #cart_ = Cart.objects.filter(customer=chat_id, product_id=p_).values_list("quantity")
    cart_ = Cart.objects.filter(customer_id=user_, product_id=p_).values_list("quantity")
    context.bot.send_message(chat_id=update.effective_chat.id, text=f"{query},\n{p},\n{x[0]},\n{p_}\n{cart_}\n{user_}\nadded to cart, Your ID {username}.")
    if not cart_:
        c = Cart(customer_id=user_, product_id=p_, quantity=3)
        c.save()
        context.bot.send_message(chat_id=update.effective_chat.id, text=f"{query},\n{p},\n{x[0]},\n{p_}\n{cart_}\nadded to cart, Your ID {username}.")
    else:
        quantity_ = Cart.objects.filter(customer_id=user_, product_id=p_).values_list("quantity")
        new_quantity = str(quantity_[0]).replace('(','').replace(')','').replace(',','')
        update_cart = int(new_quantity) + 1
        c_ = Cart(customer_id=user_, product_id=p_, quantity=update_cart)
        c_.save()
        context.bot.send_message(chat_id=update.effective_chat.id, text=f"Your ID {update_cart}\n\n{new_quantity}.")
        #new_cart = Cart(customer=c, product=int(p_), )"""
...
def add_product(update: Update, context: CallbackContext):
    ...
    user_ = str(user[0]).replace('(','').replace(')','').replace(',','')
    cart_ = Cart.objects.filter(customer_id=user_, product_id=p_).values_list("quantity")
    context.bot.send_message(chat_id=update.effective_chat.id, text=f"{cart_}.this is before")
    if cart_:
        quantity_ = Cart.objects.filter(customer_id=user_, product_id=p_).values_list("quantity")
        new_quantity = str(quantity_[0]).replace('(','').replace(')','').replace(',','')
        update_cart = int(new_quantity) + 1
        c = Cart(customer_id=user_, product_id=p_, quantity=update_cart)
        c.save()
        context.bot.send_message(chat_id=update.effective_chat.id, text=f"This is if {update_cart}\n\n{new_quantity}.")
        #new_cart = Cart(customer=c, product=int(p_), )
    else:
        c = Cart(customer_id=user_, product_id=p_, quantity=1)
        c.save()
        context.bot.send_message(chat_id=update.effective_chat.id, text=f"This is else")"""
